class ModularInt:
    ...
    def __init__(self, value, modulus):
        if not isinstance(value, int):
            raise TypeError(
                "Value must be of type int, not a {}".format(type(value).__name__)
            )
        if not isinstance(modulus, int):
            raise TypeError(
                "Modulus must be of type int, not a {}".format(type(modulus).__name__)
            )
        if modulus < 1:
    ...
    def __pow__(self, exp):
        if not isinstance(exp, int):
            raise TypeError(
                "Exponent must be of type int, not {}".format(type(value).__name__)
            )
        if exp < 0:
    ...
    def inverse(self):
        ...
        if b != 1:
            raise ValueError(
                "{} is not invertible modulo {}.".format(self.value, self.modulus)
            )
        return x0
